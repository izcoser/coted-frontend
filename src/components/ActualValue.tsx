...
const ActualValue = ({ lastDate, lastValue, lastTimestamp }: any) => {
      ...
      <div className="max-w-md border-2 rounded-xl bg-white shadow-lg mx-2 p-4 space-y-4">
        <div className="flex flex-row space-x-2 items-center">
          ...
          <Tooltip
            content={
              <span className="max-w-[32ch] block text-justify">
                Uma nova média de preços é calculada e escrita onchain a cada 2
                horas, desde que no mínimo 3 oráculos tenham reportado preços.
              </span>
            }
            placement="right"
            style={"light"}
...
function calculateRemainingTime(unixTimestamp: number) {
  ...
  const difference = 7200 - (now - unixTimestamp);
  if (difference <= 0) {
    return "00:00:00"; // The timestamp is in the past
  }
